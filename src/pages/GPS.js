...
import { MapContainer, TileLayer, Marker, Circle, LayersControl, Popup, useMapEvents } from "react-leaflet";
...
const createCustomIcon = (role, isGroup = false, isCurrentUser = false, heading = 0, distance = null) => {
  ...
  const isCloseUser = false;
  const markerColor = distance !== null ? getMarkerColor(distance, isConnectedUser, isCloseUser) : null;
    ...
    <div style="
      ...
      return `
        <div style="
          ...
          box-shadow: 0 4px 12px ${isCurrentUser ? "rgba(16,185,129,0.4)" : (markerColor ? "rgba(0,0,0,0.4)" : r.shadow)},
                      0 0 0 2px ${isCurrentUser ? "rgba(16,185,129,0.2)" : (markerColor ? "rgba(0,0,0,0.2)" : r.shadow.replace("0.4", "0.2"))};
          backdrop-filter: blur(2px); cursor: pointer;
          transition: all 0.2s ease;
          ...
          if (group.length === 1) {
            ...
            return (
              <Marker
                key={u._id}
                position={[lat, lng]}
                icon={createCustomIcon(u.role, false, false, 0, distance)}
                eventHandlers={{
