...
const Map = ({ user, locations, highlightedUserId }) => {
  ...
  useEffect(() => {
    const fetchAddressAndUpdate = async () => {
      if (userLocation) {
        try {
          const response = await axios.get(`https://nominatim.openstreetmap.org/reverse?format=jsonv2&lat=${userLocation[0]}&lon=${userLocation[1]}`);
          ...
          if (user) {
            await axios.put('/api/user/location', { location: userLocation, city: newUserInfo.city, country: newUserInfo.country, neighborhood: newUserInfo.neighborhood });
          }
        } catch (error) {
                ...
                >
                  <Popup>
                    {connectedUsers[userId] ? (
                      <>
                        <strong>{connectedUsers[userId].firstName} {connectedUsers[userId].lastName}</strong><br />
