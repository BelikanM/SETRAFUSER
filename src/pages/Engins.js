...
const socket = io("http://localhost:5000"); // ton backend
...
const Map = ({ user, locations, highlightedUserId }) => {
  ...
  // Fonction pour envoyer ta position au serveur
  const sendLocation = (pos) => {
    if (user) {
      socket.emit("updateLocation", {
        ...
        neighborhood: userInfo.neighborhood || "Inconnu",
        city: userInfo.city || "",
        country: userInfo.country || "",
        hasDevice: user.hasDevice || false,
  ...
  }, [user, userInfo.city, userInfo.country, userInfo.neighborhood]);

  // Récupère les positions des autres via socket.io
  useEffect(() => {
    socket.on("locationsUpdate", (allLocations) => {
      const newPositions = {};
      const newConnected = {};
      Object.entries(allLocations).forEach(([id, loc]) => {
        newPositions[id] = [loc.lat, loc.lng];
        newConnected[id] = {
          displayName: loc.displayName,
          neighborhood: loc.neighborhood,
          hasDevice: loc.hasDevice
        };
      });
      ...
      {userLocation ? (
        <>
          ...
          <MapContainer center={userLocation} zoom={15} style={{ height: '100%', width: '100%' }}>
            ...
            {Object.entries(positions).map(([userId, location]) => {
